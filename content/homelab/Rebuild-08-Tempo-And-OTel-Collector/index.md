...
The compose at [stack/05-tempo/compose.yaml](https://github.com/davidjudge1965/HomeLabMonitoring/blob/main/stack/05-tempo/compose.yaml):
...
The config at [stack/05-tempo/config/tempo-config.yaml](https://github.com/davidjudge1965/HomeLabMonitoring/blob/main/stack/05-tempo/config/tempo-config.yaml) is shorter than Loki's:
...
Once those three pass, Tempo is alive. The next step is wiring it into Grafana so the trace UI exists; the Tempo datasource provisioning at [stack/02-grafana/provisioning/datasources/tempo.yml](https://github.com/davidjudge1965/HomeLabMonitoring/blob/main/stack/02-grafana/provisioning/datasources/tempo.yml):
...
The Collector is stateless forwarding by design. No bind mount for state, no `.env` for per-host knobs, just a config file mounted read-only. The compose at [stack/06-otel-collector/compose.yaml](https://github.com/davidjudge1965/HomeLabMonitoring/blob/main/stack/06-otel-collector/compose.yaml):
...
The config at [stack/06-otel-collector/config/otel-collector-config.yaml](https://github.com/davidjudge1965/HomeLabMonitoring/blob/main/stack/06-otel-collector/config/otel-collector-config.yaml) has four sections: receivers, processors, exporters, service pipelines.
...
Prometheus's `/api/v1/write` endpoint is gated behind a feature flag. The diff to [stack/03-prometheus/compose.yaml](https://github.com/davidjudge1965/HomeLabMonitoring/blob/main/stack/03-prometheus/compose.yaml) is one line in the command array:
